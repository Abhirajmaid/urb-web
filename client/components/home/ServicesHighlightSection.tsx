...
import { homeServicesHighlight } from "@/data/insuranceServiceCards";
        ...
        <div className="relative bg-white rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.08)]">
          ...
          <div className="relative z-10 px-4 sm:px-6 py-8 sm:py-10 lg:px-10 xl:px-14 lg:py-12">
            ...
            <div className="mt-8 sm:mt-10 grid gap-5 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {homeServicesHighlight.map((service) => (
                <div
                  key={service.title}
                  className="rounded-3xl bg-[#ebe2df] p-6 sm:p-6 lg:p-6 text-center shadow-[0_10px_30px_-15px_rgba(0,0,0,0.25)] transition hover:-translate-y-1 flex flex-col items-center"
                >
                  <div className="mx-auto mb-4 flex h-14 w-14 shrink-0 items-center justify-center self-center rounded-full bg-[#3b1d1c] text-white sm:h-16 sm:w-16">
                    <Icon
                      icon={service.icon}
                  ...
                  <p className="mt-3 max-w-md text-left text-sm leading-snug text-[#7c6463] line-clamp-2">
                    {service.description}
                  </p>
                </div>
              ))}
